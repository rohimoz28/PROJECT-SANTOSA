...
class HrEmployeeMutation(models.Model):
    ...
    service_directorate_id = fields.Many2one('sanhrms.directorate',string='Direktorat', domain="[('branch_id', '=',service_bisnisunit)]", default=lambda self:self.employee_id.directorate_id.id)
    service_departmentid = fields.Many2one('sanhrms.department', domain="[('directorate_id', '=',service_directorate_id)]", string='Departemen', default=lambda self:self.employee_id.hrms_department_id.id)
    service_division_id = fields.Many2one('sanhrms.division',string='Divisi', domain="[('department_id', '=',service_departmentid)]", default=lambda self:self.employee_id.division_id.id)
    service_jobtitle = fields.Many2one('hr.job', domain="[('division_id', '=',service_division_id)]", string='Jabatan', index=True, default=lambda self:self.employee_id.job_id.id)
