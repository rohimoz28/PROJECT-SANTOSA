...
class SuratIzipPraktek(models.Model):
    ...
    contract_type_id = fields.Many2one(
        'hr.contract.type', "Tipe Kontrak", tracking=True)
    employee_id = fields.Many2one(
        comodel_name='hr.employee', string='Nama Karyawan', tracking=True)
    employee_name = fields.Char(
        'Nama Karyawan', related='employee_id.name', readonly=True, store=True)  # untuk filter sip
    ...
    nik_lama = fields.Char(
        string='NIK Lama', related='employee_id.nik_lama', store=True)
    area = fields.Many2one(related='employee_id.area',
                           string='Area', store=True)
    department_id = fields.Many2one(
        related='employee_id.department_id', store=True, string='Departemen')
    hrms_department_id = fields.Many2one(
        related='employee_id.hrms_department_id', store=True, string='Departemen')
    directorate_id = fields.Many2one(
        related='employee_id.directorate_id', string='Direktorat', store=True)
    division_id = fields.Many2one(
        'sanhrms.division', string='Divisi', related='employee_id.division_id', store=True)
    branch_id = fields.Many2one(
        related='employee_id.branch_id', string='Unit Bisnis', store=True)
    job_id = fields.Many2one(related='employee_id.job_id',
                             string='Jabatan', store=True)
    ...
    sip_day = fields.Integer(string='Hari Kontrak Medis',
                             compute='_compute_sip_date')
    sip_month = fields.Integer(
        string='Bulan Kontrak Medis', compute='_compute_sip_date')
    sip_year = fields.Integer(
        string='Tahun Kontrak Medis', compute='_compute_sip_date')
    ...
    salary_amount = fields.Monetary(
        string='Jumlah Gaji', currency_field='currency_id')
    ...
    def unlink_sip(self):
        for record in self:
            if record.state != 'draft':
                raise UserError(
                    "Data hanya bisa dihapus jika status SIP adalah 'New'.")
    ...
    def _compute_display_name(self):
        for account in self:
            account.display_name = account.sip_number
    ...
    def default_get(self, fields_list):
        ...
        if employee_id:
            ...
            if not employee.sip:
                raise UserError(
                    'Surat Izin Praktek hanya bisa diisi jika flag sip bernilai True / dicentang.')
        ...
        for record in self:
            if record.start_date and record.end_date:
                ...
                if record.start_date and duration_until > record.start_date:
                    ...
                    sip_duration = relativedelta(
                        duration_until, record.start_date)
