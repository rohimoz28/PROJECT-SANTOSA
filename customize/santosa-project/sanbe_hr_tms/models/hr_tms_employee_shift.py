...
class HrTMSEmployeeShift(models.Model):
    ...
    nik = fields.Char(related='employee_id.nik', string='NIK', store=True, readonly=True)
    employee_name = fields.Char(related='employee_id.name', string='Employee Name', index=True, readonly=True)
    branch_id = fields.Many2one(related='employee_id.branch_id', string='Bisnis Unit', store=True, index=True, readonly=True, default=lambda self: self.env.user.branch_id.id)
    area_id = fields.Many2one(related='employee_id.area', string='Area ID', store=True, index=True, readonly=True, default=lambda self: self.env.user.branch_id.territory_id.id)
    ...
    job_id = fields.Many2one(related='employee_id.job_id', string='Job Position', store=True, index=True, readonly=True)
    jabatan = fields.Char(related='job_id.name', string='Jabatan', store=True, index=True, readonly=True)
    ...
    name = fields.Char(string='Reference', index=True, compute='_compute_display_name', store=True)
    display_name = fields.Char(string='Display Name', index=True, compute='_compute_display_name', store=True)
    ...
    def action_review(self):
        ...
        for rec in self:
            if rec.state != 'draft' or not rec.active:
                raise UserError(_("Entri shift harus dalam status 'Draft' dan 'Active' untuk dikirim ke Review."))
    ...
    def action_approve(self):
        ...
        for rec in self:
            if rec.state != 'review' or not rec.active:
                raise UserError(_("Entri shift harus dalam status 'HRD Review' dan 'Active' untuk Disetujui."))
