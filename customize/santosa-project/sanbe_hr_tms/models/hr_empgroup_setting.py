...
class HREmpGroupSetting(models.Model):
    ...
    _inherit = ['portal.mixin', 'product.catalog.mixin',
    ...
    def _isi_department_branch(self):
        for allrecs in self:
            ...
            allwds = self.env['hr.working.days'].sudo().search(
                [('area_id', '=', allrecs.area_id.id), ('available_for', 'in', allrecs.branch_id.ids),
                 ('is_active', '=', True)])
    ...
    branch_ids = fields.Many2many('res.branch', 'res_branch_emp_rel', string='AllBranch',
                                  copy=True, compute='_isi_semua_branch', store=False, tracking=True)
    ...
    alldepartment = fields.Many2many('hr.department', 'hr_department_emp_set_rel', string='All Department',
                                     copy=True, compute='_isi_department_branch', store=False, tracking=True)
    department_id = fields.Many2one(
        'hr.department', domain="[('id','in',alldepartment)]", copy=True, string='Sub Department', tracking=True)
    ...
    state = fields.Selection([('draft', 'Draft'), ('approved', 'Approved'),
                              ('close', 'Close')], default='draft', copy=True, tracking=True)
    # valid_from = fields.Date('Valid From', required=True, copy=True)
    # valid_to = fields.Date('To', required=True, copy=True)
    summary_details_id = fields.Many2one(
        'sb.tms.tmsentry.details', string='details')
    ...
    empgroup_ids = fields.One2many('hr.empgroup.details', 'empgroup_id', auto_join=True,
                                   string='Employee Group Setting Details', copy=True, tracking=True)
    ...
    def _check_active(self):
        for rec in self:
            if rec.is_active == False:
                raise UserError(
                    'Must selected Active or In Active for this This Employee Group Setting')
    ...
    def btn_process(self):
        for res in self:
            for recx in res.empgroup_ids:
                ...
                dtlookemp = self.env['hr.empgroup.details'].sudo().search(
                    [('employee_id', '=', recx.employee_id.id), ('state', '!=', 'close')], order='valid_from asc')
                emp_list = []
                if dtlookemp:
                    for rec in dtlookemp:
                        if rec.wdcode:
                            ...
                            tmsentry = self.env['hr.attendance'].sudo().search(
                                [('employee_id', '=', rec.employee_id.id), (
                                    'area_id', '=', rec.area_id.id), ('branch_id', '=', rec.branch_id.id)],
                                order='employee_id asc, dates asc')
                            for rex in tmsentry:
    ...
    def action_search_employee(self):
        ...
        emp_line = self.env['hr.employeedepartment.details'].search(
            [('cari_id', '=', wizard.id)])
        if not emp_line:
        ...
        return {
            ...
            'target': 'new',
            'res_id': wizard.id,
            'domain': [('division_id', '=', self.division_id.id),
                       ('hrms_department_id', '=', self.hrms_department_id.id),
                       ('directorate_id', '=', self.directorate_id.id)],
            'views': [[False, 'form']]
        }
    ...
    def action_export_excel_empgroups(self, model_id=None):
        ...
        if self.hrms_department_id:
            tms_summary_domain.append(
                ('hrms_department_id', '=', self.hrms_department_id.id))
        tms_summaries = self.env['hr.empgroup.details'].search(
            tms_summary_domain)
        ...
        for vals in vals_list:
            if vals.get('name', _('New')) == _('New'):
                if 'area_id' in vals:
                    ...
                    dt_area = self.env['res.territory'].sudo().search(
                        [('id', '=', int(area))], limit=1)
                    dept = self.env['hr.department'].sudo().search(
                        [('id', '=', int(department))], limit=1)
                    department_code = dept.department_code

                    hrms_division = self.env['sanhrms.division'].sudo().search(
                        [('id', '=', int(division))], limit=1)
                    ...
                    if dt_area:
                        ...
                        vals[
                            'name'] = f"{tahun}/{bulan}/{branch_unit_id}/EG/{hrms_division_code}/{self.env['ir.sequence'].next_by_code('hr.empgroup')}"
                        # print(">>>>>>>>>>>>")
...
class HREmpGroupSettingDetails(models.Model):
    ...
    empgroup_id = fields.Many2one(
        'hr.empgroup', string='Employee Group Setting ID', index=True, tracking=True)
    empgroup_name = fields.Char(
        string='Empgroup Name', required=False, tracking=True)
    branch_ids = fields.Many2many('res.branch', 'res_branch_emp_detail_rel', string='AllBranch',
                                  copy=True, compute='_isi_semua_branch', store=False, tracking=True)
    branch_id = fields.Many2one('res.branch', string='Bisnis Unit', copy=True,
                                index=True, domain="[('id','in',branch_ids)]", tracking=True)
    area_id = fields.Many2one(
        "res.territory", string='Area ID', copy=True, index=True, tracking=True)
    area = fields.Many2one("res.territory", string='Area ID',
                           copy=True, index=True, tracking=True)
    alldepartment = fields.Many2many('hr.department', 'hr_department_emp_detail_set_rel',
                                     string='All Department', copy=True, compute='_isi_department_branch', store=False,
                                     tracking=True)
    department_id = fields.Many2one('hr.department', string='Sub Department',
                                    copy=True, index=True, domain="[('id','in',alldepartment)]", tracking=True)
    division_id = fields.Many2one(
        'sanhrms.division', string='Divisi', store=True)
    hrms_department_id = fields.Many2one(
        'sanhrms.department', string='Departemen', store=True)
    directorate_id = fields.Many2one(
        'sanhrms.directorate', string='Direktorat', store=True)
    wdcode_ids = fields.Many2many('hr.working.days', 'wd_emp_detail_rel', string='WD Code All',
                                  copy=True, compute='_isi_department_branch', store=False, tracking=True)
    wdcode = fields.Many2one(
        'hr.working.days', domain="[('id','in',wdcode_ids)]", string='WD Code', copy=True, index=True, tracking=True)
    wdcode_name = fields.Char(
        string='WD Code Name', related='wdcode.code', required=False, store=True)
    ...
    job_id = fields.Many2one(
        'hr.job', string='Job Position', index=True, tracking=True)
    valid_from = fields.Date(
        'Valid From', required=True, copy=True, tracking=True)
    ...
    state = fields.Selection([('draft', 'Draft'), ('approved', 'Approved'), ('close', 'Close')],
                             string='State', related='empgroup_id.state', store=True, tracking=True)

    def _message_log(self, body='', subject=False, message_type='notification', **kwargs):
        # print(body, subject, message_type, kwargs)
        body = "Employee Name: %s, WD Code: %s %s" % (
            self.employee_id.name, self.wdcode.code, body)
        self.empgroup_id._message_log(
            body=body, subject=subject, message_type=message_type, **kwargs)
    ...
    def create(self, values):
        for vals in values:
            if 'valid_from' in vals and 'valid_to' in vals and 'employee_id' in vals:
                tfrom = (datetime.strptime(
                    str(vals['valid_from']), "%Y-%m-%d").date())
                tto = (datetime.strptime(
                    str(vals['valid_to']), "%Y-%m-%d").date())
                demp = self.env['hr.empgroup.details'].sudo().search(
                    [('empgroup_id', '!=', False), ('employee_id', '=', int(
                        vals['employee_id'])), ('valid_from', '<=', tfrom), ('valid_to', '>=', tfrom)])
                empgroup_name = demp.empgroup_id.name if demp.empgroup_id else "Unknown Group"
                if demp:
                    raise UserError(
                        f'Valid From overlaps with existing data in EMP Group: {empgroup_name}')
                demp = self.env['hr.empgroup.details'].sudo().search(
                    [('empgroup_id', '!=', False), ('employee_id', '=', int(
                        vals['employee_id'])), ('valid_from', '<=', tto), ('valid_to', '>=', tto)])
                if demp:
                    raise UserError(
                        f'Valid to over with existing data in EMP Group: {empgroup_name}')
    ...
    def cek_tgl(self):
        for rec in self:
            ...
            if rec.valid_from > rec.valid_to:
                raise UserError(
                    'Date Valid To must be bigger then Date Valid From')
            datecek = self.env['hr.empgroup.details'].sudo().search(
                [('empgroup_id', '!=', False), ('id', '!=', rec.id), (
                    'employee_id', '=', rec.employee_id.id), ('valid_from', '<=', tfrom), ('valid_to', '>=', tfrom)])
            if datecek:
                raise UserError('Valid From over lap with existing data')
            datecek = self.env['hr.empgroup.details'].sudo().search(
                [('empgroup_id', '!=', False), ('id', '!=', rec.id), (
                    'employee_id', '=', rec.employee_id.id), ('valid_from', '<=', tto), ('valid_to', '>=', tto)])
