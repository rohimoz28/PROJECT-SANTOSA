...
class HrCariEmployeeShift(models.TransientModel):
    ...
    branch_id = fields.Many2one('res.branch', string='Bisnis Unit', index=True,
                                domain="[('id','in',branch_ids)]", default=lambda self: self.env.user.branch_id.id)
    department_id = fields.Many2one('hr.department', domain="[('id','in',alldepartment)]", string='Sub Department',
                                    index=True, default=lambda self: self.env.user.employee_id.department_id.id)
    ...
    empgroup_id = fields.Many2one(
        'hr.empgroup',
        string='Cari Employee Group', domain="[('state','=','approved'),('branch_id','=',branch_id)]",
    )
    ...
    def action_process(self):
        ...
        for line in self:
            if line.target_process == 'generate':
                # message = 'Apakah Anda yakin ingin menghapus data yang ada?'
                # result = self.env['ir.actions.act_window'].with_context(
...
class HrCariEmployeeShiftDetails(models.TransientModel):
    ...
    cari_id = fields.Many2one(
        'wiz.employee.shift', string='Employee Cari ID', ondelete='cascade', index=True)
    branch_id = fields.Many2one('res.branch', string='Bisnis Unit', index=True,
                                domain="[('id','in',branch_ids)]", related='cari_id.branch_id')
    employee_id = fields.Many2one(
        'hr.employee', string='Employee Name', index=True)
