...
class HrEmployeeContractMonitoring(models.Model):
    ...
    hrms_department_id = fields.Many2one(comodel_name='sanhrms.department', string='Departemen', store=True)
    ...
    def new_contract(self, type, old_contract):
        ...
        for existing_contract in self.env['hr.contract'].search(
                [('employee_id', '=', self.employee_id.id), ('state', '=', 'open')]):
            existing_contract.state = 'close'
        if type == 'promotion' and (
                old_contract.structure_type_id.name != 'Employee' or old_contract.structure_type_id.name != 'Permanent' or old_contract.structure_type_id.name != 'Full-Time'):
            new_type = self.env['hr.contract.type'].search([('name', '=', 'Permanent')])
            new_status = new_type.id
        else:
            new_status = old_contract.type_id.id
        month = datetime.now().month
        contracts_branch = self.env['hr.contract'].search([('name', 'ilike',
                                                            '/HC-' + str(self.territory_id.area_code) + '/' + str(
                                                                self.branch_id.branch_code) + '/' + month_to_roman(
        ...
        return self.env['hr.contract'].sudo().create({
            ...
            'name': number_contract + '/HC-' + str(self.territory_id.area_code) + '/' + str(
                self.branch_id.branch_code) + '/' + month_to_roman(month) + '/' + str(datetime.now().year),
    ...
    def mail_send_hr(self):
        ...
        query_superior_id = """
                            SELECT DISTINCT hcm.branch_id
                            FROM hr_employee_contract_monitoring hcm
                            where hcm.time_limit <= 90; \
                            """
        ...
        for distinct_records in self.env.cr.dictfetchall():
            ...
            table_rows = ""
            idx = 1
            for records in self.env['hr.employee.contract.monitoring'].search(
                    [('branch_id', '=', distinct_records['branch_id']), ('time_limit', '<=', 90)]):
                table_rows += f"""
                    <tr>
                        <td>{idx}</td> 
...
class HrContractMonitoring(models.Model):
    ...
    superior_id = fields.Many2one('hr.employee', 'Immediate Superior', related='employee_id.parent_id', readonly=True,
                                  store=True)
    ...
    branch_id = fields.Many2one('res.branch', related='employee_id.branch_id', string='Business Unit', readonly=True,
                                store=True)
    branch_name = fields.Char(related='branch_id.name', string='Business Unit', readonly=True, store=True)
    territory_id = fields.Many2one('res.territory', related='employee_id.area', string='Area', readonly=True,
                                   store=True)
    ...
    hrms_department_id = fields.Many2one(comodel_name='sanhrms.department', related='employee_id.hrms_department_id',
    ...
    def _generate_name(self):
        for line in self:
            if line.employee_id:
                name = str(fields.Datetime.today().year)[-2:] + ('0' + str(fields.Datetime.today().month))[
                    -2:] + line.employee_id.nik
                return name
    ...
    def generate_data(self):
        ...
        for employee in self.env['hr.employee'].search([]):
            ...
            end_date = (datetime.today().replace(day=1) + relativedelta(months=1) - timedelta(days=1)).strftime(
                "%Y-%m-%d")
    ...
    def mail_send_hr(self):
        # try:
        list_employe_contract = self.env['hr.contract.monitoring'].search(
            [('is_done', '=', 'waiting'), ('company_id', '=', self.company_id.id)])
        distinct_records = []
        for branch_id, group in groupby(list_employe_contract, key=lambda r: r.branch_id.id):
            ...
            for record in distinct_records:
                query = """ SELECT hcm.id,
                                   ...
                                   sd.name::VARCHAR AS department_name,   -- Single-language field for department hj.id AS job_id,
                                   hj.name::VARCHAR AS job_name,          -- Single-language field for job he.name parent_name,
                                   hcm.employee_name,
                                   hc.date_end::date date_end
                            FROM hr_contract_monitoring hcm
                                     LEFT JOIN
        ...
        for data_superiors in self.env.cr.dictfetchall():
            ...
            for record in self.env.cr.dictfetchall():
                ...
                table_rows += (("""
                    ...
                    """) % (record['territory_name'], record['branch_name'], department_name, record['employee_name'],
                            job_name, record['parent_name'] or '', record['date_end']))
            email_body = (("""
                <p>Semangat Pagi <br/>
                Mentor/Supervisor %s </p> %s %s %s  <p>Human Resource</p>""") % (mentor_name, table_header, table_rows,
                                                                                 table_footer))
            email = self.env.ref('sanbe_hr_monitoring_contract.email_template_reminder_contract_end')
