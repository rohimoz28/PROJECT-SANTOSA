...
class HrResignation(models.Model):
    ...
    job_status = fields.foo = fields.Selection(
        related='employee_id.job_status')
    ...
    effective_date = fields.Date('Tanggal Berlaku')
    ...
    end_contract = fields.Boolean(
        string='Rehire', default=False, store=True, help="Penanda pe-rekrutan ulang karyawan")
    # rehire = fields.Boolean(
    #     'Rehire', default=False, store=True, help="Penanda pe-rekrutan ulang karyawan")
    ...
    def _compute_employee_data(self):
        """ Memastikan data tersimpan di database saat employee_id berubah """
        for record in self:
    ...
    @api.depends('contract_id', 'state', 'employee_id', 'job_status')
    ...
    @api.onchange('contract_id', 'employee_id', 'state')
    ...
    def action_confirm_resignation(self):
        ...
        for alldata in self:
            ...
            # log_id = self.env['hr.employment.log'].search(
            #     [('employee_id', '=', alldata.employee_id.id), ('trx_number', '=', alldata.name)], limit=1)
            # if log_id:
            #     log_id.end_date = alldata.effective_date
            #     log_id.end_contract = alldata.rehire
        return res
    ...
    def write(self, vals_list):
        ...
        for allrec in self:
            if allrec.state == 'open':
                allrec.joined_date = allrec.join_date or allrec.first_date_join
