...
class HomeFace(Home):
    ...
    def web_login(self, redirect=None, **kw):
        ...
        if background == 'color':
            ...
            # bg_src = f"background-image:linear-gradient(#ffcc00, #ff3300)"
            color1 = param_obj.get_param('login_background.color')
            bg_src = f"background-color:{color1};"
            # values['background_src'] = "linear-gradient(#ffcc00, #ff3300)"
        ...
        if style == 'default' or style is False:
            ...
            response = request.render('sanbe_login.middle_login_template', values)
